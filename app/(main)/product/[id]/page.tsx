...
const ProductDetailsPage = () => {
  ...
  return (
    <div className="bg-[#f3f3f3] py-8">
      <div className="mx-auto grid w-full max-w-[1240px] grid-cols-1 gap-6 px-4 lg:grid-cols-[220px_1fr_1fr]">
        <aside className="space-y-3 rounded border border-gray-300 bg-white p-3">
          <h3 className="font-semibold">Featured Product</h3>
          {products.slice(0, 5).map((p) => (
            <Link key={p.id} href={`/product/${p.id}`} className="flex items-center gap-2 border-b border-gray-100 pb-2">
              <img src={p.image} alt={p.name} className="h-12 w-12 object-contain" />
              <span className="text-xs text-gray-600">{p.name.slice(0, 20)}...</span>
            </Link>
          ))}
        </aside>
        ...
        <section className="rounded border border-gray-300 bg-white p-4">
          ...
          <div className="mt-3 flex gap-2">
            {[product.image, product.hoverImage, products[2].image].map((src, idx) => (
              <img key={idx} src={src} alt="thumb" className="h-16 w-16 rounded border border-gray-300 object-cover" />
            ))}
          </div>
        </section>

        <section className="rounded border border-gray-300 bg-white p-4">
          <p className="text-xs text-gray-500">Brand: Creative</p>
          <h1 className="mt-1 text-3xl text-[38px] font-semibold text-gray-800">{product.name}</h1>
          <p className="mt-2 text-4xl text-[46px] font-bold text-[#ef553f]">{taka(product.price)}</p>
          <div className="mt-2 flex items-center gap-2">
            <Stars rating={product.rating} /> <span className="text-sm text-gray-500">(2 reviews)</span>
          </div>
          <p className="mt-4 text-sm text-gray-600">{product.description}</p>
          <p className="mt-4 text-sm text-green-600">{product.inStock} in stock</p>
          <div className="mt-5 flex items-center gap-3">
            <button className="rounded border border-gray-300 px-3 py-2">-</button>
            <span>1</span>
            <button className="rounded border border-gray-300 px-3 py-2">+</button>
            <button onClick={() => addToCart(product)} className="rounded bg-[#ef553f] px-10 py-2 font-semibold text-white">
              Add To Cart
            </button>
          </div>
          <button onClick={() => addToWishlist(product)} className="mt-4 block text-sm text-gray-700">
            Wishlist
          </button>
        </section>
      </div>
